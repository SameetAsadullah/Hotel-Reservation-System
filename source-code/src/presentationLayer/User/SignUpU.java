...
public class SignUpU {
    ...
    @FXML
    void actionPerformed(ActionEvent event) { // submit button to submit all the values
        HRS hrs = new HRS();
        ...
        else if (!hrs.validateCustomerEmail(email)) {
        ...
        if (!name_invalid.isVisible() && !email_invalid.isVisible() && !cnic_invalid.isVisible() && !contact_invalid.isVisible() &&
            ...
            hrs.registerCustomer(name, email, pass, address, contact, cnic, credit);
            ((Stage) (((Button) event.getSource()).getScene().getWindow())).close();
